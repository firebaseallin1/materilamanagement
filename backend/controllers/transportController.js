const { Transport } = require('../models/Transactions');
...
exports.create = async (req, res) => {
  try {
    const doc = await Transport.create({ ...req.body, createdBy: req.user.id });
...
exports.remove = async (req, res) => {
  try { await Transport.findByIdAndDelete(req.params.id); res.json({ success: true, message: 'Deleted' }); }
  catch (err) { res.status(500).json({ success: false, message: err.message }); }
};
