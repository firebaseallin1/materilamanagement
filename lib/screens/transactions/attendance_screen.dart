...
class _AttendanceScreenState extends State<AttendanceScreen> {
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Expanded(flex: 2, child: Text(date,
              style: const TextStyle(fontSize: 13, color: Color(0xFF374151)))),
          SizedBox(width: 60, child: Center(child: _hrsBadge('${presentHrs}h', isPresent))),
          SizedBox(width: 60, child: Center(
            child: Text(otHours > 0 ? '${otHours % 1 == 0 ? otHours.toInt() : otHours}h' : '—',
                style: TextStyle(
                    fontSize: 13,
                    fontWeight: otHours > 0 ? FontWeight.w600 : FontWeight.normal,
                    color: otHours > 0 ? const Color(0xFF7C3AED) : const Color(0xFF9CA3AF))),
          )),
          ...
          SizedBox(width: 88, child: Text(
            ...
            style: TextStyle(
                ...
                color: total > 0 ? const Color(0xFF111827) : const Color(0xFF9CA3AF)),
          )),
          _actionMenu(item),
        ]),
      ),
  ...
  Widget _hrsBadge(String label, bool present) => Container(
    ...
    child: Text(label, style: TextStyle(
        ...
        color: present ? const Color(0xFF16A34A) : const Color(0xFFDC2626))),
  );
...
class _AttendanceFormPanelState extends State<AttendanceFormPanel> {
  ...
  Future<void> _save() async {
    ...
    final body = {
      ...
      'otHours': _otEnabled ? (double.tryParse(_otHoursCtrl.text.trim()) ?? 0) : 0,
  ...
  Widget build(BuildContext context) {
    ...
    final otHrs = _otEnabled ? (double.tryParse(_otHoursCtrl.text.trim()) ?? 0) : 0.0;
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (_otEnabled) ...[
                ...
                TextFormField(
                  ...
                  keyboardType: const TextInputType.numberWithOptions(decimal: true),
                  inputFormatters: [FilteringTextInputFormatter.allow(RegExp(r'^\d*\.?\d*'))],
                  decoration: _dec('OT Hours', Icons.more_time_rounded, hint: 'e.g. 2'),
                  onChanged: (_) => setState(() {}),
                  validator: (v) {
              ...
              if (rate > 0)
                Container(
                  ...
                  child: Column(children: [
                    ...
                    Row(children: [
                      const Text('Total', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF166534))),
                      const Spacer(),
              ...
              Row(children: [
                ...
                Expanded(flex: 2, child: DecoratedBox(
                  decoration: BoxDecoration(
                    gradient: _saving ? null : const LinearGradient(colors: [_primary, Color(0xFF1E6F5C)],
  ...
  Widget _summaryRow(String label, String detail, String value) => Row(children: [
    Text(label, style: const TextStyle(fontSize: 12, color: Color(0xFF374151))),
    const SizedBox(width: 6),
    Text(detail, style: const TextStyle(fontSize: 11, color: Color(0xFF6B7280))),
    const Spacer(),
    Text(value, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(0xFF166534))),
  ]);
}
